...
ExplodingSprite::ExplodingSprite(const Sprite& s) :
  ...
  chunks(), 
  freeList(),
  totalChunks(Gamedata::getInstance().getXmlInt(getName()+"Chunks"))
  {
  makeChunks();
}
...
void ExplodingSprite::update() { 
  ...
  while (ptr != chunks.end()) {
    ptr->update();
    if (ptr->outOfBounds() || ptr->getDistance() > 100) {
...
Frame* ExplodingSprite::makeChunkFrames() { 

  unsigned chunk_width = frame->getWidth()/totalChunks;
  unsigned chunk_height = frame->getHeight()/totalChunks;
  Sint16 source_x = frame->getSourceX();
  Sint16 source_y = frame->getSourceY();

  void *rawMemory = operator new[](totalChunks*totalChunks*sizeof(Frame));
  Frame* frames = static_cast<Frame*>(rawMemory);
  unsigned count = 0;
  for (unsigned i = 0; i < totalChunks; ++i) {
    for (unsigned j = 0; j < totalChunks; ++j) {
