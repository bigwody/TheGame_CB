...
IOManager::IOManager( ) 
  ...
  : screen(SDL_SetVideoMode( Gamedata::getInstance().getXmlInt("viewWidth"), Gamedata::getInstance().getXmlInt("viewHeight"), 0, SDL_HWSURFACE)),
...
void IOManager::printMessageCenteredAt( const string& msg, Uint32 y) const {
   ...
   SDL_Surface *stext = TTF_RenderText_Blended(font, msg.c_str(), color);
   if (stext) {
     Uint32 x = ( Gamedata::getInstance().getXmlInt("viewWidth") - stext->w ) / 2;
     SDL_Rect dest = {x,y,0,0};
     SDL_BlitSurface( stext, NULL, screen, &dest );
     SDL_FreeSurface(stext);
   }
...
void IOManager::buildString(SDL_Event event) {
  if( inputString.size() <= Gamedata::getInstance().getXmlUnsignedInt("StringMaxSize")  ) {
